...
class LCGlobalVerifyData(object):
    ...
    i_lc_global_bhm_percent = 0  # 总揽测量成单率
    i_lc_global_newvip_num = 0  # 总揽新增会员数
    i_lc_global_newvip_order_percent = 0  # 总揽新会员成单率
    i_lc_global_order_num = 0  # 总揽订单数
    i_lc_global_refund_num = 0  # 总揽退单数
    i_lc_global_sale_num = 0  # 总揽销售总额
    i_lc_global_sale_percent = 0  # 总揽销售完成比
...
class LCGlobal(IDataVerify):
    ...
    def data_verify(self):
        ...
        assert abs(
            self.postVerifyData.i_lc_global_sale_num - self.expectedData.i_lc_global_sale_num - self.preVerifyData.i_lc_global_sale_num) == 0, \
            "总揽销售总额检测失败,期待增加值:%d, 当前值:%d, 之前值:%d" % (
                self.expectedData.i_lc_global_sale_num, self.postVerifyData.i_lc_global_sale_num, self.preVerifyData.i_lc_global_sale_num)
